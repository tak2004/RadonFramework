...
{
public:
  void Execute(RF_Collect::Tree<RF_Mem::AutoPointer<Token>>::NodeType& Parent,
...
{
public:
  ReplaceTokenCommand(RF_Mem::AutoPointer<Token>& AToken) : NewToken(AToken) {}
...
{
  decltype(m_AST)::NodeType* current = nullptr;
...
void Tokenizer::ParseCode(RF_Mem::AutoPointerArray<RF_Type::UInt8>& Code)
{
  auto hits = RF_Algo::FindAll(
      Code, [=](RF_Mem::AutoPointerArray<RF_Type::UInt8>::ConstEnumeratorType&
                    Element) {
  ...
  if(hits.Count() > 0)
  {
    ...
    if(hits[hits.Count() - 1] < Code.Count())
    {
      token.Reset(new CodePart(hits[hits.Count() - 1],
                               Code.Count() - hits[hits.Count() - 1]));
      currentNode->Data() = token;
      currentNode = currentNode->Next();
    }
  }
...
void Tokenizer::ApplyChanges(
    RF_Collect::Tree<RF_Mem::AutoPointer<Token>>::NodeType*& Node,
    RF_Collect::List<RF_Mem::AutoPointer<ASTCommand>>& Todo)
{
  RF_Type::Int32 offset = 0;
  RF_Collect::Tree<RF_Mem::AutoPointer<Token>>::NodeType* parent =
      Node->Parent();
...
{
  RF_Collect::Stack<RF_Collect::Tree<RF_Mem::AutoPointer<Token>>::NodeType*>
      stack;
  for(RF_Type::Size i = 0; i < m_Rules.Count(); ++i)
  {
    decltype(m_AST)::NodeType* current = nullptr;
    ...
    while(stack.Size() > 0)
    {
      if(!m_Rules[i]->Visit(*current))
      {
        return;
      }

      // apply changes and go downwards need special treatment
      if(current->Childs() > 0 && stack.Size() > 1 &&
         m_Rules[i]->Todo.Count() > 0)
      {
      ...
      {  // go up
        while(stack.Size() > 0 && stack.Peek()->Next() == nullptr)
        {
          ApplyChanges(current, m_Rules[i]->Todo);
          m_Rules[i]->Todo.Clear();
          stack.Pop();
                         ...
                         const TokenNode& Last)
{
  ...
  if(First.Parent()->IndexOf(First, ChildStart) &&
     Last.Parent()->IndexOf(Last, ChildEnd) && First.Parent() == Last.Parent())
  {
    RF_Mem::AutoPointer<ASTCommand> cmd(
        new MoveIntoCommand(ChildStart, ChildEnd));
