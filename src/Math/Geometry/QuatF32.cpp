...
{
  RF_Type::Float32 a =
      RF_Math::Float32::DegToRad(Alpha * static_cast<RF_Type::Float32>(0.5));
  RF_ASSERT(Direction.Length() >= RF_Math::Float32::EPSILION,
         "Invalid parameter.");
...
QuatF32::QuatF32(const RF_Type::Float32 w,
                         const RF_Type::Float32 X,
                         const RF_Type::Float32 Y,
                         const RF_Type::Float32 Z)
{
  m_Vec[0] = w;
  m_Vec[1] = X;
  m_Vec[2] = Y;
...
QuatF32::QuatF32(const Matrix<RF_Type::Float32, 3, 3>& RotationMatrix)
{
  ...
  if(tr >= 0.0)
  {
    s = RF_Math::Float32::Sqrt(tr + static_cast<RF_Type::Float32>(1)) *
        static_cast<RF_Type::Float32>(2);
  ...
  else
  {
    ...
    {
      case 0:
        s = RF_Math::Float32::Sqrt(
            RotationMatrix[0] - (RotationMatrix[4] + RotationMatrix[8]) +
            static_cast<RF_Type::Float32>(1.0));
      ...
      case 4:
        s = RF_Math::Float32::Sqrt(
            RotationMatrix[4] - (RotationMatrix[8] + RotationMatrix[0]) +
            static_cast<RF_Type::Float32>(1.0));
      ...
      case 8:
        s = RF_Math::Float32::Sqrt(
            RotationMatrix[8] - (RotationMatrix[0] + RotationMatrix[4]) +
            static_cast<RF_Type::Float32>(1.0));
...
QuatF32::QuatF32(const RF_Type::Float32 Roll, const RF_Type::Float32 Pitch,
                         const RF_Type::Float32 Yaw)
{
  RF_Type::Float32 r =
      RF_Math::Float32::DegToRad(Roll * static_cast<RF_Type::Float32>(0.5));
  RF_Type::Float32 p =
      RF_Math::Float32::DegToRad(Pitch * static_cast<RF_Type::Float32>(0.5));
  RF_Type::Float32 y =
      RF_Math::Float32::DegToRad(Yaw * static_cast<RF_Type::Float32>(0.5));
...
{
  QuatF32 quat;
  quat.m_Vec[0] = m_Vec[0] * Other.m_Vec[0] - m_Vec[1] * Other.m_Vec[1] -
                  m_Vec[2] * Other.m_Vec[2] - m_Vec[3] * Other.m_Vec[3];
  quat.m_Vec[1] = m_Vec[0] * Other.m_Vec[1] + m_Vec[1] * Other.m_Vec[0] +
                  m_Vec[2] * Other.m_Vec[3] - m_Vec[3] * Other.m_Vec[2];
  quat.m_Vec[2] = m_Vec[0] * Other.m_Vec[2] + m_Vec[2] * Other.m_Vec[0] +
                  m_Vec[3] * Other.m_Vec[1] - m_Vec[1] * Other.m_Vec[3];
  quat.m_Vec[3] = m_Vec[0] * Other.m_Vec[3] + m_Vec[3] * Other.m_Vec[0] +
                  m_Vec[1] * Other.m_Vec[2] - m_Vec[2] * Other.m_Vec[1];
  return quat;
}
...
QuatF32& QuatF32::Invert()
{
  ...
  RF_ASSERT(val != static_cast<RF_Type::Float32>(0.0), "Invalid value.");
  Conjugate().m_Vec /= val;
  return *this;
}
...
{
  QuatF32 result;
  RF_Type::Float32 angle = RF_Math::Float32::Sqrt(m_Vec[1] * m_Vec[1] + m_Vec[2] * m_Vec[2] + m_Vec[3] * m_Vec[3]);
...
{
  if(m_Vec[0] == 0.0 && m_Vec[1] == 0.0 && m_Vec[2] == 0.0 && m_Vec[3] == 0.0)
    return 0.0;
...
void QuatF32::ToAxisAngle(Vector<RF_Type::Float32, 3>& Axis,RF_Type::Float32& Angle)
{
  ...
  Angle = RF_Math::Float32::RadToDeg(
      static_cast<RF_Type::Float32>(2) * RF_Math::Float32::ACos(m_Vec[0]));
