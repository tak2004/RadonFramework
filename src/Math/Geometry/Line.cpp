...
Line2D::Line2D(const Int32 X1,const Int32 Y1, const Int32 X2, const Int32 Y2)
{
...
Bool Line2D::Intersection(const Line2D &Other, Point2D<> &Point)const
{
  Float32 distAB, theCos,theSin,newX,ABpos;
  Point2D<> p1,p2,p3,p4;
  ...
  ABpos=(Float32)(p4.X+(p3.X-p4.X)*p4.Y)/(Float32)(p4.Y-p3.Y);
...
Bool Line2D::IsIntersect(const Line2D &Other)const
{
  Point2D<> p1;
...
Bool Line2D::IsIntersect(const Point2D<> &Other)const
{
  //If the point lay inside the boundingbox the check can do with Pythagoras law.
  if (BoundingBox().IsIntersect(Other))
  {
    Float32 u,x,y;
    //looking for delta on the line
    u=(Float32)((Other-m_Point1)*(m_Point2-m_Point1)).Sum()/(Float32)m_Point1.SquareDistance(m_Point2);
    //neares point on the line
...
Float32 Line2D::PointDistance(const Point2D<> &Other)const
{
  return sqrt(PointSquareDistance(Other));
}

Float32 Line2D::PointSquareDistance(const Point2D<> &Other)const
{
  Float32 u,x,y;
  //looking for delta on the line
  u=(Float32)((Other.X-m_Point1.X)*(m_Point2.X-m_Point1.X)+(Other.Y-m_Point1.Y)*(m_Point2.Y-m_Point1.Y))/(Float32)m_Point1.SquareDistance(m_Point2);
  //neares point on the line
  x=m_Point1.X+u*(m_Point2.X-m_Point1.X);
  y=m_Point1.Y+u*(m_Point2.Y-m_Point1.Y);
  //clamp to line
...
Float32 Line2D::Length()const
{
  return m_Point1.Distance(m_Point2);
}

Float32 Line2D::SquareLength()const
{
  return (Float32) m_Point1.SquareDistance(m_Point2);
}
