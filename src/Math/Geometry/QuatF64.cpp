...
{
  RF_Type::Float64 a =
      RF_Math::Float64::DegToRad(Alpha * static_cast<RF_Type::Float64>(0.5));
  RF_ASSERT(Direction.Length() >= RF_Math::Float64::EPSILION,
         "Invalid parameter.");
...
QuatF64::QuatF64(const RF_Type::Float64 w,
                         const RF_Type::Float64 X,
                         const RF_Type::Float64 Y,
                         const RF_Type::Float64 Z)
{
  m_Vec[0] = w;
  m_Vec[1] = X;
  m_Vec[2] = Y;
...
QuatF64::QuatF64(const Matrix<RF_Type::Float64, 3, 3>& RotationMatrix)
{
  ...
  if(tr >= 0.0)
  {
    s = RF_Math::Float64::Sqrt(tr + static_cast<RF_Type::Float64>(1)) *
        static_cast<RF_Type::Float64>(2);
  ...
  else
  {
    ...
    {
      case 0:
        s = RF_Math::Float64::Sqrt(
            RotationMatrix[0] - (RotationMatrix[4] + RotationMatrix[8]) +
            static_cast<RF_Type::Float64>(1.0));
...
QuatF64::QuatF64(const RF_Type::Float64 Roll, const RF_Type::Float64 Pitch,
                         const RF_Type::Float64 Yaw)
{
  RF_Type::Float64 r =
      RF_Math::Float64::DegToRad(Roll * static_cast<RF_Type::Float64>(0.5));
  RF_Type::Float64 p =
      RF_Math::Float64::DegToRad(Pitch * static_cast<RF_Type::Float64>(0.5));
  RF_Type::Float64 y =
      RF_Math::Float64::DegToRad(Yaw * static_cast<RF_Type::Float64>(0.5));
...
{
  QuatF64 result;
  RF_Type::Float64 angle = RF_Math::Float64::Sqrt(m_Vec[1] * m_Vec[1] + m_Vec[2] * m_Vec[2] + m_Vec[3] * m_Vec[3]);
...
void QuatF64::ToAxisAngle(Vector<RF_Type::Float64, 3>& Axis,RF_Type::Float64& Angle)
{
  ...
  Angle = RF_Math::Float64::RadToDeg(
      static_cast<RF_Type::Float64>(2) * RF_Math::Float64::ACos(m_Vec[0]));
