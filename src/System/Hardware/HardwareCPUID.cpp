...
void DecodeCacheInfo(CacheInfo& CacheInfo, RF_Type::UInt32 reg)
{
    reg &= 0xff;
    ...
    case 0x0a: case 0x1d: case 0x3b: case 0x7f:
        CacheInfo.Associativity = CacheAssociativity::_2WaySetAssociative;
        break;
    ...
    case 0xca: case 0xd0: case 0xd1: case 0xd2:
        CacheInfo.Associativity = CacheAssociativity::_4WaySetAssociative;
        break;

    case 0x0e: case 0x1a: case 0x3a: case 0x3d:
        CacheInfo.Associativity = CacheAssociativity::_6WaySetAssociative;
        break;
    ...
    case 0xd6: case 0xd7: case 0xd8:
        CacheInfo.Associativity = CacheAssociativity::_8WaySetAssociative;
        break;

    case 0x48: case 0x4a: case 0x4c: case 0x8d: case 0xdc: case 0xdd: case 0xde:
        CacheInfo.Associativity = CacheAssociativity::_12WaySetAssociative;
        break;

    case 0x24: case 0x49: case 0x4b: case 0x4d: case 0xe2: case 0xe3: case 0xe4:
        CacheInfo.Associativity = CacheAssociativity::_16WaySetAssociative;
        break;

    case 0x4e: case 0xea: case 0xeb: case 0xec:
        CacheInfo.Associativity = CacheAssociativity::_24WaySetAssociative;
        break;
    ...
    case 0xa0:
        CacheInfo.Associativity = CacheAssociativity::FullyAssociative;
        break;

    default:
        CacheInfo.Associativity = CacheAssociativity::Other;
    }
...
RF_Type::Bool GetLogicalProcessorFeatures(ProcessorFeatureMask& Features)
{
    ...
    {
        CPUId(1, reg);
        Features[ProcessorFeatures::AES] = (reg[2] & BITMASK_AES) != 0;
        Features[ProcessorFeatures::AVX] = (reg[2] & BITMASK_AVX) != 0;
        Features[ProcessorFeatures::CLFLUSH] = (reg[3] & BITMASK_CLFLUSH) != 0;
        Features[ProcessorFeatures::CMOV] = (reg[3] & BITMASK_CMOV) != 0;
        Features[ProcessorFeatures::CX16] = (reg[2] & BITMASK_CX16) != 0;
        Features[ProcessorFeatures::CX8] = (reg[3] & BITMASK_CX8) != 0;
        Features[ProcessorFeatures::FMA] = (reg[2] & BITMASK_FMA) != 0;
        Features[ProcessorFeatures::FMOV] = (reg[3] & BITMASK_FMOV) != 0;
        Features[ProcessorFeatures::FPU] = (reg[3] & BITMASK_FPU) != 0;
        Features[ProcessorFeatures::HTT] = (reg[3] & BITMASK_HTT) != 0;
        Features[ProcessorFeatures::MMX] = (reg[3] & BITMASK_MMX) != 0;
        Features[ProcessorFeatures::MOVBE] = (reg[2] & BITMASK_MOVBE) != 0;
        Features[ProcessorFeatures::PCLMUL] = (reg[2] & BITMASK_PCLMUL) != 0;
        Features[ProcessorFeatures::POPCNT] = (reg[2] & BITMASK_POPCNT) != 0;
        Features[ProcessorFeatures::SSE] = (reg[3] & BITMASK_SSE) != 0;
        Features[ProcessorFeatures::SSE2] = (reg[3] & BITMASK_SSE2) != 0;
        Features[ProcessorFeatures::SSE3] = (reg[2] & BITMASK_SSE3) != 0;
        Features[ProcessorFeatures::SSSE3] = (reg[2] & BITMASK_SSSE3) != 0;
        Features[ProcessorFeatures::SSE4_1] = (reg[2] & BITMASK_SSE4_1) != 0;
        Features[ProcessorFeatures::SSE4_2] = (reg[2] & BITMASK_SSE4_2) != 0;
        Features[ProcessorFeatures::TSC] = (reg[3] & BITMASK_TSC) != 0;
        result = true;
    }
    if(ids > 6)
    {
    ...
    if(ids > 0x80000000)
    {
        Features[ProcessorFeatures::SSE4A] = (reg[2] & BITMASK_SSE4A) != 0;
        Features[ProcessorFeatures::FMA4] = (reg[2] & BITMASK_FMA4) != 0;
        Features[ProcessorFeatures::XOP] = (reg[2] & BITMASK_XOP) != 0;
    }
    return result;
}
...
#if RF_BUILD_INTRINSIC_CPUID
    GetCacheCount = CPUID::DetectCacheCount;
    GetLogicalProcessorFeatures = CPUID::GetLogicalProcessorFeatures;
#endif
#ifdef RF_WINDOWS
