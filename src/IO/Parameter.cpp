...
Bool Parameter::SafeParsingWithErrorOutput(
    const AutoPointerArray<String>& Parameters,
    ...
    String& ErrorMessage)
{
  ...
  for(UInt32 i = 0; i < Rules.Count(); ++i)
  {
    ...
    if(Parameters.Count() > parameterIndex + parameterOffset)
    {
      ...
      {
        matchingParameter = Parameters[parameterIndex] ==
                            String::UnsafeStringCreation(Rules[i].LongName);
        if(Rules[i].ShortName != 0)
          matchingParameter |= Parameters[parameterIndex] ==
                               String::UnsafeStringCreation(Rules[i].ShortName);
      }
      else
      {
        if(Rules[i].ShortName != 0)
        ...
        case OptionRule::Required:  // going crazy
        {
          if(matchingParameter)
          {
            ErrorMessage = String::Format(
                String("Can't parse '%s', expected '%s' or '%s'."),
                              ...
                              const AutoPointerArray<OptionRule>& Rules)
{
  ...
  {
    param[i] = String::UnsafeStringCreation(
...
RF_Type::Bool
Parameter::ParsingWithErrorOutput(const char* argv[],
                                  int argc,
                                  const AutoPointerArray<OptionRule>& Rules,
