...
RF_Type::String File::NameWithoutExtension() const 
{
  String result;
  auto path = m_Location.GetComponents(UriComponents::Path);
  auto filename = path.Split(Uri::PathSeperator).Last();
  auto delimiterIndex = filename.LastIndexOf('.');
...
RF_Type::String File::Extension() const 
{
  ...
  auto filename = path.Split(Uri::PathSeperator).Last();
  auto delimiterIndex = filename.LastIndexOf('.');
  return filename.SubString(delimiterIndex, filename.Length() - delimiterIndex);
}
...
AutoPointerArray<UInt8> File::Read()
{
  ...
  FileStream stream;
  stream.Open(m_Location, RF_SysFile::FileAccessMode::Read,
              RF_SysFile::FileAccessPriority::ReadThroughput);
