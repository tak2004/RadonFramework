...
AutoPointerArray<String> Directory::Files() const
{
  ...
  {
    if(!RF_SysFile::Stat(systemPath + RF_SysFile::Seperator() + content[i])
            ->IsDirectory)
    {
...
{
  return FilesIncludingSubdirectories(RF_Collect::List<String>());
}
...
AutoPointerArray<String> Directory::FilesIncludingSubdirectories(
    const RF_Collect::List<String>& Skip) const
{
  RF_Type::String systemPath;
  RF_SysFile::UriToSystemPath(m_Uri, systemPath);
  auto resolvedSkipPaths = Skip;
  for(auto& entry : resolvedSkipPaths)
  {
    auto fullPath = systemPath + RF_SysFile::Seperator() + entry;
...
AutoPointerArray<String>
Directory::FilesIncludingSubdirectories(const RF_Collect::List<String>& Skip,
                                        const RF_Type::String& BasePath) const
{
  ...
  for(auto& entry : resolvedSkipPaths)
  {
    auto fullPath = BasePath + RF_SysFile::Seperator() + entry;
    RF_SysFile::RealPath(fullPath, entry);
  }
...
AutoPointer<File> Directory::SubFile(const RF_Type::String& Filename) const
{
  ...
  if(Exists())
  {
    ...
    auto location =
        Uri(Location().OriginalString() + RF_SysFile::Seperator() + Filename);
    result->SetLocation(location);
  }
  return result;
}
