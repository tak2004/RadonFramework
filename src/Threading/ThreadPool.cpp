...
public:
  PoolTask();
  ~PoolTask();
  PoolTask(ThreadPool::WaitCallback Callback,
           void* Data,
...
public:
  Data()
  {
    ...
    MaxWorkerThreads =
        lps;  // ThreadPool::GetBestThreadAmountByProcessorCoreAmount(lps);
...
{
  return RF_Pattern::Singleton<ThreadPool>::GetInstance();
}

UInt32 ThreadPool::GetBestThreadAmountByProcessorCoreAmount(UInt32 Amount)
{
                                   ...
                                   FreeCallback FreeData)
{
  if(m_PImpl->IsQueingAllowed)
  {
    ...
    {
      Int64 serialGrp = RF_SysHardware::GetCurrentUniqueProcessorNumber() %
                        m_PImpl->WorkerThreads.Count();
