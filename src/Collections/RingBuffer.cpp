...
public:
    ...
    RF_Type::Bool Write(RF_Type::UInt8* Data, const RF_Type::Size Bytes)
    {
        ...
        auto writeableBytes = ((m_UsableBytes + m_ReadCursor) - currentWriteCursor) % m_UsableBytes;
        while(Bytes < writeableBytes)
        {
            auto lastWriteCursor = m_WriteCursor.CompareAndExchange(currentWriteCursor,
                (currentWriteCursor + Bytes) % m_UsableBytes);
            ...
            writeableBytes = ((m_UsableBytes + m_ReadCursor) - currentWriteCursor) % m_UsableBytes;
        }
        return false;
    }
private:
...
public:
    ...
    RF_Type::Bool Read(RF_Type::UInt8* Data, RF_Type::Size Bytes)
    {
        ...
        auto readableBytes = ((m_UsableBytes + m_WriteCursor) - m_ReadCursor) % m_UsableBytes;
        while(Bytes <= readableBytes)
        {
            auto lastReadCursor = m_ReadCursor.CompareAndExchange(currentReadCursor,
                (currentReadCursor + Bytes) % m_UsableBytes);
            ...
            readableBytes = ((m_UsableBytes + m_WriteCursor) - currentReadCursor) % m_UsableBytes;
        }
        return false;
    }
    ...
    RF_Type::Bool Write(RF_Type::UInt8* Data, const RF_Type::Size Bytes)
    {
        auto writeableBytes = ((m_UsableBytes + m_ReadCursor) - m_WriteCursor) % m_UsableBytes;
        if(Bytes < writeableBytes)
        {
            RF_SysMem::Copy(m_View + m_WriteCursor, Data, Bytes);
            m_WriteCursor = (m_WriteCursor + Bytes) % m_UsableBytes;
