...
// The elements per bucket are determined by 
    template <class KEYTYPE = u32, IsContinousGrowableAllocator ALLOCATOR= Allocator> class Hashlist {
    public:
        Hashlist() = delete;
...
:allocator(LongLiving), stack(Stack){
    ...
    auto offset = (MemoryLimit / (this->hashesGrowSize + this->valuesGrowSize)) * this->hashesGrowSize;
...
void Hashlist<KEYTYPE, ALLOCATOR>::clone(Hashlist& To) const
{
    ...
    auto offset = reinterpret_cast<ptrdiff>(this->values) - reinterpret_cast<ptrdiff>(this->pool.address);
...
{
    auto block = this->allocator.commit(this->pool, this->hashesSize, this->hashesGrowSize);
    this->hashesSize += block.bytes;
    auto byteOffset = reinterpret_cast<ptrdiff>(this->values) - reinterpret_cast<ptrdiff>(this->pool.address);
    block = this->allocator.commit(this->pool, byteOffset + this->valuesSize, this->valuesGrowSize);
...
inline size Hashlist<KEYTYPE, ALLOCATOR>::find(KEYTYPE Key)const
{
    ...
    if (this->buckets) {
        ...
        keys.memory = slice(this->pool, offset*this->hashesGrowSize, this->hashesGrowSize);
