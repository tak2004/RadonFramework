...
DateTime DateTime::CreateByTicks(timeval Value, DateTimeKind Kind) {
  DateTime result;
  result.refDelta = TimeSpan::CreateByTicks(Value);
  result.kind = Kind;
  return result;
}
...
DateTime DateTime::CreateByTime(u32 Year, u32 Month, u32 Day, u32 Hour,
  ...
  if (Hour >= 0 && Hour < 24 && Minute >= 0 && Minute < 60 && Second >= 0 &&
    ...
    if (Day == 0 && Month == 0 && Year == 0) {
      result.refDelta = TimeSpan::CreateByTime(0, Hour, Minute, Second, 0, 0);
    } else {
      if (Year >= 0 && Year < 10000 && Month >= 0 && Month < 13 && Day >= 0 &&
          Day <= DaysInMonth(Year, Month)) { // is datetime
        result.refDelta = TimeSpan::CreateByTime(AbsoluteDays(Year, Month, Day),
...
DayOfWeek DateTime::getDayOfWeek() const {
  return static_cast<DayOfWeek>((refDelta.days() + 1) % 7);
}
...
u32 DateTime::hour() const { return refDelta.hours(); }

u32 DateTime::minute() const { return refDelta.minutes(); }

u32 DateTime::second() const { return refDelta.seconds(); }

u32 DateTime::millisecond() const { return refDelta.milliseconds(); }

u32 DateTime::microsecond() const { return refDelta.microseconds(); }

TimeSpan DateTime::timeOfDay() const {
  TimeSpan result = TimeSpan::CreateByTicks(refDelta.ticks % 1_d);
  return result;
}

TimeSpan DateTime::asTimeSpan() const {
  TimeSpan ts(refDelta);
  return ts;
}
...
bool DateTime::LessThan(DateTime t1, DateTime t2) {
  return t1.refDelta < t2.refDelta;
}
