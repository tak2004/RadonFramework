...
public:
  ...
  using LinearAllocator =
      RF_Mem::LinearAllocator<RF_Mem::HeapAllocator, 4096, 16>;
  ...
  : TestSuite("RadonFramework::Memory::LinearAllocator"_rfs)
  {
    ...
    AddTest(
        MakeDelegate(this, &LinearAllocatorTest::DoCPlusPlusAllocationNoPOD),
  ...
  void SetUp()
  {
    allocatedIntList = RF_Mem::AutoPointerArray<int*>(new int*[INTALLOCATIONS],
                                                      INTALLOCATIONS);
    allocatedPODList = RF_Mem::AutoPointerArray<PODStruct*>(
        new PODStruct*[STUCTALLOCATIONS], STUCTALLOCATIONS);
    allocatedNoPODList = RF_Mem::AutoPointerArray<NoPOD*>(
        new NoPOD*[CLASSALLOCATIONS], CLASSALLOCATIONS);
    pageSize = RF_SysMem::GetPageSize();
    reserveBytesForIntStack =
        (((sizeof(int) + sizeof(RF_Type::Size)) * INTALLOCATIONS) / pageSize +
         1) *
        pageSize;
    reserveBytesForPODStack =
        (((sizeof(PODStruct) + sizeof(RF_Type::Size)) * STUCTALLOCATIONS) /
             pageSize +
         1) *
        pageSize;
    reserveBytesForNoPODStack =
        (((sizeof(NoPOD) + sizeof(RF_Type::Size)) * CLASSALLOCATIONS) /
             pageSize +
         1) *
        pageSize;
  }
