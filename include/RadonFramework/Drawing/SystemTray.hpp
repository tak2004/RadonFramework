...
{
public:
    typedef RF_SysDraw::SystemTrayService::Handle Handle;
    ...
    /// Request the dimension at which a TrayIcon will be represented in the SystemTray.
    void RequestTrayIconSize(RF_Geo::Size2D<>& WriteTo)const;

    /// Return true if the SystemTray is supported on the running system else false.
    static RF_Type::Bool IsSupported();
protected:
    friend class RF_Pattern::Singleton<SystemTray>;
    SystemTray();
    virtual ~SystemTray() override;
