...
public:
  ...
  /// memory allocation. The specified instance will not be destroyed on
  /// destruction.
...
private:
  ...
  RF_Mem::FreeListAllocator<ALLOCATOR, sizeof(T), alignof(T)>
      m_PayloadAllocator;
};
...
inline void DynamicQueueMPSC<T, ALLOCATOR>::Clear()
{
  ...
  while(p = (m_RealQueue.Dequeue()))
  {
    ...
    RF_Mem::MemoryBlock block = {reinterpret_cast<RF_Type::UInt8*>(p),
                                 sizeof(T)};
