...
protected:
  ...
  RF_Collect::List<RF_Mem::AutoPointerArray<RF_Type::UInt8>>
      m_ManagedSubCommands;
  void ExecuteCommandChain(RF_Mem::AutoPointerArray<RF_Type::UInt8>& Command);
};

template <typename K /*=RF_Type::UInt32*/>
template <class T, class CMD>
T* CommandBucket<K>::AppendCommand(
    const CMD& Command,
    const RF_Type::Size AdditionalCommandMemory /*= 0*/)
{
  static_assert(std::is_pointer<CMD>::value == false,
                "CMD must be an object reference!");
  static_assert(std::is_pod<T>::value == true, "T have to be a POD structure!");

  // find tail
  CommandHeader* tail =
      reinterpret_cast<CommandHeader*>(const_cast<CMD*>(&Command)) - 1;
  ...
  // add to tail
  RF_Type::Size bytes =
      sizeof(T) + sizeof(CommandHeader) + AdditionalCommandMemory;
  m_ManagedSubCommands.AddLast(RF_Mem::AutoPointerArray<RF_Type::UInt8>(bytes));
  CommandHeader* header =
      reinterpret_cast<CommandHeader*>(m_ManagedSubCommands.Last()->Get());
                                ...
                                const RF_Type::Size AdditionalCommandMemory)
{
  ...
  m_Keys.AddLast(Key);
  CommandHeader* header =
      reinterpret_cast<CommandHeader*>(m_Commands.Last()->Get());
  header->Child = nullptr;
  header->DispatchFunction = T::DispatchFunction;
  header->AdditionalCommandMemory = AdditionalCommandMemory;
  return reinterpret_cast<T*>(header + 1);
...
RF_Type::Size CommandBucket<K>::GetCommandMemorySize(const T& Command)
{
  const CommandHeader* header =
      reinterpret_cast<const CommandHeader*>(&Command) - 1;
...
template <typename K /*=RF_Type::UInt32*/>
void CommandBucket<K>::ExecuteCommandChain(
    RF_Mem::AutoPointerArray<RF_Type::UInt8>& Command)
{
