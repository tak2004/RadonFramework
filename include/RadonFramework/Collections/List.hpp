...
public:
  ...
  using ConstIterator = ListIterator<T, Node, const T*, const T&>;
  using Type = T;
  friend Iterator;
...
template <typename T>
List<T>::Node::Node() : m_Next(0), m_Prev(0)
{
}

template <typename T>
List<T>::Node::Node(T& V) : m_Next(nullptr), m_Prev(nullptr), m_Value(V)
{
}
...
template <typename T, class Node, class Pointer, class Reference>
ListIterator<T, Node, Pointer, Reference>::ListIterator(Node* node,
                                                        List<T>* Owner)
...
: m_Node(It.m_Node), m_Owner(It.m_Owner)
{
}

template <typename T, class Node, class Pointer, class Reference>
ListIterator<T, Node, Pointer, Reference>&
ListIterator<T, Node, Pointer, Reference>::operator=(const ListIterator& It)
{
  if(&It != this)
  {
    m_Node = It.m_Node;
...
template <typename T, class Node, class Pointer, class Reference>
RF_Type::Bool ListIterator<T, Node, Pointer, Reference>::
operator==(const ListIterator& It) const
{
...
template <typename T, class Node, class Pointer, class Reference>
ListIterator<T, Node, Pointer, Reference>&
ListIterator<T, Node, Pointer, Reference>::operator++()
{
...
template <typename T, class Node, class Pointer, class Reference>
ListIterator<T, Node, Pointer, Reference>
ListIterator<T, Node, Pointer, Reference>::operator++(int)
{
...
template <typename T, class Node, class Pointer, class Reference>
ListIterator<T, Node, Pointer, Reference>&
ListIterator<T, Node, Pointer, Reference>::operator+=(const RF_Type::Size Step)
{
...
template <typename T, class Node, class Pointer, class Reference>
const ListIterator<T, Node, Pointer, Reference>
ListIterator<T, Node, Pointer, Reference>::
operator+(const RF_Type::Size Step) const
{
  Node* n = m_Node;
  for(RF_Type::Size i = 0; i < Step; ++i)
    if(n->GetNext())
...
template <typename T, class Node, class Pointer, class Reference>
RF_Type::Size
ListIterator<T, Node, Pointer, Reference>::Distance(ListIterator Start,
                                                    ListIterator Stop)
{
  RF_Type::Size i = 0;
  auto it = Start;
...
template <typename T>
List<T>::List(const List<T>& Copy) : m_First(nullptr), m_Last(nullptr)
{
  AddList(Copy);
}
...
typename List<T>::Iterator List<T>::Begin() const
{
  return Iterator(m_First, const_cast<List<T>*>(this));
}

template <typename T>
typename List<T>::ConstIterator List<T>::ConstEnd() const
{
  return ConstIterator(0, const_cast<List<T>*>(this));
}

template <typename T>
typename List<T>::ConstIterator List<T>::ConstBegin() const
{
  return ConstIterator(m_First, const_cast<List<T>*>(this));
}
