...
{
  explicit ExistsEnumeratorTaskData(FUNCTION function) : Function(function) {}
  FUNCTION Function;
...
void ExistsEnumeratorTaskFunction(void* Data)
{
  ...
  RF_Type::Size i, end;
  for(i = 0, end = data->Steps; i < end && (*data->Hits) == 0;
      ++i, ++data->Enumeable)
  {
     ...
     elements > 1)
  {
    ...
    for(RF_Type::UInt32 i = 0, offset = 0; i < worker; ++i)
    {
      ...
      RF_Pattern::Singleton<RF_Thread::ThreadPool>::GetInstance()
          .QueueUserWorkItem(ExistsEnumeratorTaskFunction<C, FUNCTION>, task);
