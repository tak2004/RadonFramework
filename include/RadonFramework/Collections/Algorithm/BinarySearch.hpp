...
RF_Type::Size BinarySearch(ArrayEnumerator<T>& Range, const T& Value)
{
    ...
    while(current->MoveTo((static_cast<RF_Type::MemoryRange>(upperBorderIterator->Current() - lowerBorderIterator->Current()) >> 1) +
        static_cast<RF_Type::MemoryRange>(lowerBorderIterator->Current() - Range.Current()) &&
