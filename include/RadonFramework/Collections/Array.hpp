...
public:
  using Type = T;
  using EnumeratorType = Enumerator<T, ArrayEnumeratorType>;
  using ConstEnumeratorType = Enumerator<const T, ArrayEnumeratorType>;
   ...
   *                Array to create.
   */
  explicit Array(const RF_Type::Size Length1, const RF_Type::Size Length2);
   ...
   */
  static Memory::AutoPointer<Array<T, MA, MO>>
  CreateInstance(const T* CArray, const RF_Type::Size Length);
   ...
   */
  RF_Type::Bool
  ConstrainedCopy(Array<T, MA, MO>& DestinationArray, RF_Type::Size Length);
  ...
  Memory::AutoPointer<Array<Out>>
  ConvertAll(const Delegate1<Out(T&)>& Converter);
   ...
   *               the number of elements to copy.
   */
  void Copy(Array<T, MA, MO>& DestinationArray, RF_Type::Size Length);
   ...
   *              the elements to search for.
   */
  Memory::AutoPointer<Array<T, MA, MO>>
  FindAll(const Delegate1<RF_Type::Bool(const T&)>& Match);
   ...
   *              the element to search for.
   */
  RF_Type::Size FindIndex(const Delegate1<RF_Type::Bool(const T&)>& Match);
   ...
   *              the element to search for.
   */
  RF_Type::Size FindLastIndex(const Delegate1<RF_Type::Bool(const T&)>& Match);
   ...
   *              the element to search for.
   */
  RF_Type::Size FindLastIndex(RF_Type::Size StartIndex,
                              const Delegate1<RF_Type::Bool(const T&)>& Match);
   ...
   *               element to get.
   */
  const T& GetValue(RF_Type::Size Index1, RF_Type::Size Index2);
   ...
   */
  const T&
  GetValue(RF_Type::Size Index1, RF_Type::Size Index2, RF_Type::Size Index3);
   ...
   *         otherwise, –1.
   */
  RF_Type::Size IndexOf(const T& Value, const RF_Type::Size StartIndex) const;
   ...
   */
  RF_Type::Size
  LastIndexOf(const T& Value, const RF_Type::Size StartIndex) const;
   ...
   */
  void Reverse(RF_Type::Size Index, RF_Type::Size Length);
   ...
   */
  void SetValue(const T& Value, Array<RF_Type::Size>& Indices);
   ...
   *               element to set.
   */
  void SetValue(const T& Value, RF_Type::Size Index1, RF_Type::Size Index);
   ...
   *         value is true.
   */
  RF_Type::Bool TrueForAll(const Delegate1<RF_Type::Bool(const T&)>& Match);
   ...
   * elements.
   */
  Array<T, MA, MO>& operator=(const Array<T, MA, MO>& Other);

  T& operator()(const RF_Type::Size Index);

  const T& operator()(const RF_Type::Size Index) const;

  T& operator()(const RF_Type::Size Index1, const RF_Type::Size Index2);
...
protected:
  ...
  RF_Type::Bool InitArray(RF_Type::Size Rank, const RF_Type::Size* LengthArray);
};
...
inline void Array<T, MA, MO>::Exchange(const RF_Type::Size IndexA,
                                       const RF_Type::Size IndexB)
{
...
template <typename T, typename MA, typename MO>
Memory::AutoPointer<Array<T, MA, MO>>
Array<T, MA, MO>::CreateInstance(const T* CArray, const RF_Type::Size Length)
{
  typename Memory::AutoPointer<Array<T, MA, MO>> arr(
      MA::template New<Array<T, MA, MO>>());
...
template <typename T, typename MA, typename MO>
RF_Type::Bool
Array<T, MA, MO>::ConstrainedCopy(RF_Type::Size SourceIndex,
                                  Array<T, MA, MO>& DestinationArray,
                                  RF_Type::Size DestinationIndex,
                                  RF_Type::Size Length)
{
...
template <typename T, typename MA, typename MO>
RF_Type::Bool
Array<T, MA, MO>::ConstrainedCopy(Array<T, MA, MO>& DestinationArray,
                                  RF_Type::Size Length)
{
...
template <typename Out>
Memory::AutoPointer<Array<Out>>
Array<T, MA, MO>::ConvertAll(const Delegate1<Out(T&)>& Converter)
{
  Array<RF_Type::Size, MA, MO> lengths(m_Rank);
  for(RF_Type::Size i = 0; i < m_Rank; ++i)
    lengths.m_Data[i] = m_Length[i];

  Memory::AutoPointer<Array<Out, MA, MO>> result =
      Array<Out, MA, MO>::CreateInstance(lengths);
...
template <typename T, typename MA, typename MO>
RF_Type::Bool
Array<T, MA, MO>::Exists(const Delegate1<RF_Type::Bool(const T&)>& Match)
{
...
template <typename T, typename MA, typename MO>
Memory::AutoPointer<Array<T, MA, MO>>
Array<T, MA, MO>::FindAll(const Delegate1<RF_Type::Bool(const T&)>& Match)
{
...
template <typename T, typename MA, typename MO>
typename Array<T, MA, MO>::ConstEnumeratorType
Array<T, MA, MO>::GetConstEnumerator() const
{
...
void Array<T, MA, MO>::Resize(RF_Type::Size NewSize)
{
  ...
  if(NewSize != m_Length[0])
  {
    ...
    if(std::is_trivially_copyable<T>::value == false)
    {
      ...
      for(RF_Type::Size i = 0, end = RF_Math::Integer<RF_Type::Size>::Min(
                                   m_Length[0], NewSize);
          i < end; ++i)
      {
        data[i] = m_Data[i];
    ...
    {
      MO::Copy(data, m_Data,
               RF_Math::Integer<RF_Type::Size>::Min(m_Length[0], NewSize));
    }
#endif
    MA::FreeArray(m_Data);
    m_Data = data;
...
template <typename T, typename MA, typename MO>
RF_Type::Bool
Array<T, MA, MO>::TrueForAll(const Delegate1<RF_Type::Bool(const T&)>& Match)
{
  for(RF_Type::Size i = 0; i < m_ElementCount; ++i)
...
template <typename T, typename MA, typename MO>
RF_Type::Bool Array<T, MA, MO>::InitArray(RF_Type::Size Rank,
