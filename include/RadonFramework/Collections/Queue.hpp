...
: m_Tail(0), m_Head(0), m_LastHead(0), m_LastTail(0)
{
  m_LPInfoCount = RF_SysHardware::GetAvailableLogicalProcessorCount();
  m_LPInfos = reinterpret_cast<LPInfo*>(
      RF_SysMem::Allocate(sizeof(LPInfo) * m_LPInfoCount, 4096));
  RF_SysMem::Fill(m_LPInfos, &RF_Type::UInt32Max, sizeof(RF_Type::UInt32),
                  sizeof(LPInfo) * m_LPInfoCount);

  m_Array = reinterpret_cast<T*>(
      RF_SysMem::Allocate(sizeof(T) * ReserveElements, 4096));
