...
template <typename T>
AutoPointer<T>::AutoPointer(ElementType* Ptr) : m_Data(Ptr)
{
}

template <typename T>
AutoPointer<T>::AutoPointer(AutoPointer& Copy) : m_Data(Copy.Release())
{
}
...
AutoPointer<T>::operator AutoPointer<T1>()
{
  static_assert(std::is_class<T>::value == false ||
                    (std::is_class<T>::value == true &&
                     std::is_class<T1>::value == true &&
                     std::is_base_of<T1, T>::value == true),
                "T and T1 are classes but not related to each other.");
