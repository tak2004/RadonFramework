...
{
public:
  static RF_Mem::AutoPointer<Socket>
  Create(RF_SysNet::NetService::SocketHandler& Handler);

  static RF_Mem::AutoPointer<Socket>
  Create(const AddressFamily Family, const SocketType Type);

  static RF_Mem::AutoPointer<Socket>
  Create(const AddressFamily Family, const SocketType Type, SocketError& Error);
  ...
  SocketError Connect(const IPAddress& IP, const RF_Type::UInt32 Port);
  ...
  SocketError SetSocketOption(const SocketOptionLevel OptionLevel,
                              ...
                              const RF_Type::Bool Value);
  SocketError SetSocketOption(const SocketOptionLevel OptionLevel,
                              const SocketOptionName OptionName,
                              const RF_Type::UInt8 Value);
  SocketError SetSocketOption(const SocketOptionLevel OptionLevel,
                              const SocketOptionName OptionName,
                              const RF_Type::Int32 Value);
  SocketError SetSocketOption(const SocketOptionLevel OptionLevel,
                              const SocketOptionName OptionName,
