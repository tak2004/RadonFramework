...
template <typename T, RF_Type::Size N, RF_Type::Size M>
inline T& Matrix<T, N, M>::
operator()(RF_Type::Size Col, RF_Type::Size Row) const
{
  RF_ASSERT(Col * M + Row >= 0 && Col * M + Row < N * M, "Index out of bound.");
...
{
  RF_ASSERT(Index < N * M, "Index out of bound.");
  return Value[Index];
}
...
{
  RF_ASSERT(Col * 2 + Row < 4, "Index out of bound.");
...
Matrix<T, 2, 2> Matrix<T, 2, 2>::operator*(Matrix<T, 2, 2>& Other) const
{
  ...
  for(RF_Type::Size i = 0; i < 2; ++i)
    for(RF_Type::Size j = 0; j < 2; ++j)
      out[i * 2 + j] =
          (*this)[i * 2] * Other[j] + (*this)[i * 2 + 1] * Other[2 + j];
  return out;
}
...
{
  RF_ASSERT(Col * 3 + Row < 9, "Index out of bound.");
...
{
  RF_ASSERT(Index < 9, "Index out of bound.");
  return Value[Index];
}
...
Matrix<T, 3, 3> Matrix<T, 3, 3>::operator*(Matrix<T, 3, 3>& Other) const
{
  ...
  for(RF_Type::Size i = 0; i < 3; ++i)
    for(RF_Type::Size j = 0; j < 3; ++j)
      out[i * 3 + j] = (*this)[i * 3] * Other[j] +
                       (*this)[i * 3 + 1] * Other[3 + j] +
                       (*this)[i * 3 + 2] * Other[6 + j];
  return out;
}
...
Matrix<T, 3, 3>& Matrix<T, 3, 3>::Rotate(T w, T x, T y, T z)
{
  ...
  T len = v.Length();
  RF_ASSERT(len != 0,
         "Argument is not legal because the x,y,z components are zero.");
...
{
  return Value[0] * Value[4] * Value[8] + Value[3] * Value[7] * Value[2] +
         Value[6] * Value[1] * Value[5] - Value[6] * Value[4] * Value[2] -
         Value[0] * Value[7] * Value[5] - Value[3] * Value[1] * Value[8];
}
