...
public:
  CubicBezier();
  CubicBezier(const Point2D<TIN>& Start,
              const Point2D<TIN>& StartControl,
              const Point2D<TIN>& EndControl,
              const Point2D<TIN>& End);
  ...
  static RF_Type::Bool IsClockwise(const Point2D<TIN>& A,
                                   const Point2D<TIN>& B,
                                   const Point2D<TIN>& C);
  static RF_Type::Bool IsInsideCircle(const Point2D<TIN>& A,
                                      const Point2D<TIN>& B,
                                      const Point2D<TIN>& C,
                                      const Point2D<TIN>& D);

  Curve::Type GetType(TOUT& Det1, TOUT& Det2, TOUT& Det3, TOUT& Det4) const;
  void GenerateTriangles(RF_Collect::List<Vector<TOUT, 4>>& Vertice,
...
CubicBezier<TIN, TOUT>::CubicBezier()
{
}

template <class TIN, class TOUT>
CubicBezier<TIN, TOUT>::CubicBezier(const Point2D<TIN>& Start,
                                    const Point2D<TIN>& StartControl,
                                    const Point2D<TIN>& EndControl,
                                    const Point2D<TIN>& End)
{
...
template <class TIN, class TOUT>
RF_Type::Bool CubicBezier<TIN, TOUT>::IsClockwise(const Point2D<TIN>& A,
                                                  const Point2D<TIN>& B,
                                                  const Point2D<TIN>& C)
{
  TOUT edge0[2];
...
template <class TIN, class TOUT>
RF_Type::Bool CubicBezier<TIN, TOUT>::IsInsideCircle(const Point2D<TIN>& A,
                                                     const Point2D<TIN>& B,
                                                     const Point2D<TIN>& C,
                                                     const Point2D<TIN>& D)
{
  ...
  TOUT dysq = D.Y * D.Y;
  Matrix<TOUT, 3, 3> det;
  det.SetColumn(0, Vector<TOUT, 3>(A.X - D.X, A.Y - D.Y,
                                   A.X * A.X - dxsq + A.Y * A.Y - dysq));
  det.SetColumn(1, Vector<TOUT, 3>(B.X - D.X, B.Y - D.Y,
                                   B.X * B.X - dxsq + B.Y * B.Y - dysq));
  det.SetColumn(2, Vector<TOUT, 3>(C.X - D.X, C.Y - D.Y,
                                   C.X * C.X - dxsq + C.Y * C.Y - dysq));
...
template <class TIN, class TOUT>
Curve::Type CubicBezier<TIN, TOUT>::GetType(TOUT& Det1,
                                            TOUT& Det2,
                                            TOUT& Det3,
                                            TOUT& Det4) const
{
  ...
  b.SetColumn(3, v4);
  b.Transpose();
  b *= m3;  // use internal Transpose for faster calculation, do a transpose
            // before and after
  b.Transpose();
  ...
  {
    if(di > 0.0)
      return Curve::Serpentine;
    if(di < 0.0)
      return Curve::Loop;
    if(di == 0.0)
      return Curve::CuspInflectionInfinity;
  }
  else
  {
    if(Det3 != 0.0)
      return Curve::CuspWithCuspInfinity;
    else
    {
      if(Det4 != 0.0)
...
template <class TIN, class TOUT>
void CubicBezier<TIN, TOUT>::GenerateTriangles(
    ...
    RF_Collect::List<Vector<TOUT, 4>>& UVs)
{
  ...
  switch(ct)
  {
    case Curve::Serpentine:
    {
      ...
      k1 = Vector<TOUT, 4>(-sm * tl - sl * tm, -3.0 * sl * tl * tl,
                           -3.0 * sm * tm * tm, 0.0);
      k2 =
          Vector<TOUT, 4>(sl * sm, 3.0 * sl * sl * tl, 3.0 * sm * sm * tm, 0.0);
      ...
      Vertice.AddLast(
          Vector<TOUT, 4>(StartPointControl.X, StartPointControl.Y));
    ...
    case Curve::Loop:
    {
      ...
      k1 = Vector<TOUT, 4>(-se * td - sd * te,
                           -se * td * td - 2.0 * sd * te * td,
                           -sd * te * te - 2.0 * se * td * te, 1.0);
      k2 = Vector<TOUT, 4>(sd * se, te * sd * sd + 2.0 * se * td * sd,
                           td * se * se + 2.0 * sd * te * se, 1.0);
...
template <class TIN, class TOUT>
void CubicBezier<TIN, TOUT>::GeneratePoints(
    RF_Collect::List<Point2D<TOUT>>& points,
    RF_Type::UInt32 Steps)
{
  for(RF_Type::UInt32 i = 1; i <= Steps; i++)
  {
    RF_Type::Float32 t = static_cast<RF_Type::Float32>(i) / Steps;
    Point2D<TOUT> T(t, t);
    Point2D<TOUT> T1(1.0f - t, 1.0f - t);

    Point2D<TOUT> U = T1 * StartPoint + T * StartPointControl;
    Point2D<TOUT> V = T1 * StartPointControl + T * EndPointControl;
    Point2D<TOUT> W = T1 * EndPointControl + T * EndPoint;

    Point2D<TOUT> M = T1 * U + T * V;
    Point2D<TOUT> N = T1 * V + T * W;
