...
template<class TIN, class TOUT>
CubicBezier<TIN, TOUT>::CubicBezier(const Point2D<TIN>& Start, const Point2D<TIN>& StartControl,
    const Point2D<TIN>& EndControl, const Point2D<TIN>& End)
{
    StartPoint = Start;
...
template<class TIN, class TOUT>
RF_Type::Bool CubicBezier<TIN, TOUT>::IsClockwise(const Point2D<TIN>& A, const Point2D<TIN>& B,
    const Point2D<TIN>& C)
{
    TOUT edge0[2];
    TOUT edge1[2];
...
template<class TIN, class TOUT>
RF_Type::Bool CubicBezier<TIN, TOUT>::IsInsideCircle(const Point2D<TIN>& A, const Point2D<TIN>& B,
    const Point2D<TIN>& C, const Point2D<TIN>& D)
{
    TOUT dxsq = D.X*D.X;
    TOUT dysq = D.Y*D.Y;
...
template<class TIN, class TOUT>
Curve::Type CubicBezier<TIN, TOUT>::GetType(TOUT& Det1, TOUT& Det2, TOUT& Det3, TOUT& Det4)const
{
    Matrix<TOUT, 4, 4> m3;
...
template<class TIN, class TOUT>
void CubicBezier<TIN, TOUT>::GenerateTriangles(RF_Collect::List<Vector<TOUT, 4> >& Vertice,
    RF_Collect::List<Vector<TOUT, 4> >& UVs)
{
    TOUT d1, d2, d3, d4;
...
template<class TIN, class TOUT>
void CubicBezier<TIN, TOUT>::GeneratePoints(RF_Collect::List<Point2D<TOUT> >& points, RF_Type::UInt32 Steps)
{
