...
public:
  ...
  Vector(T a, T b)
  {
    // if you get an error on this line you use to much parameter for a too
    // small vector
...
#undef CompareOperator
  ...
  T Distance(const Vector& Other) const { return ((*this) - Other).Length(); }
  ...
  Vector CrossProduct(const Vector& Vec) const
  {
    ...
    for(RF_Type::Size i = 0; i < SIZE; ++i)
    {
      ...
      nextnext = (i + 2) % SIZE;
      v.Value[i] =
          Value[next] * Vec.Value[nextnext] - Value[nextnext] * Vec.Value[next];
    }
    return v;
  }
...
using Vec2f = Vector<RF_Type::Float32, 2>;
using Vec3f = Vector<RF_Type::Float32, 3>;
using Vec4f = Vector<RF_Type::Float32, 4>;
