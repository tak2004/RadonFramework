...
Rectangle<PT, ST>::Rectangle(const Point2D<PT>& Position,
                             const Size2D<ST>& Size)
: m_Position(Position.X, Position.Y), m_Size(Size.Width, Size.Height)
{
}
...
template <class PT, class ST>
Rectangle<PT, ST>::Rectangle(const PT Left,
                             const PT Top,
                             const PT Right,
                             const PT Bottom)
: m_Position(Left, Top), m_Size(Right - Left, Bottom - Top)
{
}

template <class PT, class ST>
Rectangle<PT, ST>::Rectangle(const Point2D<PT>& Point1,
                             const Point2D<PT>& Point2)
{
...
template <class PT, class ST>
RF_Type::Bool
Rectangle<PT, ST>::IsIntersect(const Rectangle<PT, ST>& Other) const
{
  auto tmp(m_Position);
  tmp.X += m_Size.Width;
  tmp.Y += m_Size.Height;
...
using Rectanglef = Rectangle<RF_Type::Float32, RF_Type::Float32>;
