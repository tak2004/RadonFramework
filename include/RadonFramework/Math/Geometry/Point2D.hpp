...
template <class T>
Point2D<T>::Point2D(const T x, const T y) : X(x), Y(y)
{
}
...
{
  return static_cast<RF_Type::Float32>(((X - Other.X) ^ 2) +
                                       ((Y - Other.Y) ^ 2));
}

using Point2Df = Point2D<RF_Type::Float32>;
using Point2Dd = Point2D<RF_Type::Float64>;
