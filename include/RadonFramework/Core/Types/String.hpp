 ...
 *
 * The String class use a 32Bit unsigned integer as length type(4294967295
 *characters as maximum). This class consist of three variables, first m_Length,
 *second m_Text and third m_DataManagment. The m_Text variable is a array of
 *char which is 0 terminated. The m_DataManagament specify how the data should
 *be managed. E.g. constant c-strings are global and can't be cleaned up but if
 *you want to work with them then you can specify the data as UnmanagedInstance
 *and the object don't try to clean it up.
 **/
...
#pragma region Constructor and Destructor
   ...
   * @param CStringSize Contains the size of the CString including the 0
   *termination.
   **/
  explicit String(const char* CString,
                  const Size CStringSize,
   ...
   */
  static String
  UnsafeStringCreation(const char* CString,
                       DataManagment Ownership = DataManagment::Copy);
#pragma endregion
...
#pragma region Properties
   ...
   *
   * Attention: Don't change the Data of this pointer and don't typecast it to
   *"char*". Because this data point on the buffer of this String class and
   *isn't a copy. The reason for this function is to provide a very fast
   *interface to C based libraries like Operating System API's.
   **/
...
#pragma region Methods
   ...
   *
   * @return Return -1 if Str isn't contain or greater -1(position where the
   *string was found).
   *
   **/
   ...
   *
   * @return Return -1 if the String isn't part of this String else the start
   *position.
   **/
  Int32 IndexOf(const String& Value) const;

  /** @brief Check if a String is part of this String after the position of
   *StartAtIndex.
   *
   * @return Return -1 if the String isn't part of this String else the start
   *position.
  ...
  /** @brief Check if a String is part of this String after the position of
   *StartAtIndex till the number of Count steps is reached.
   *
   * StartAtIndex mark the start of the search and Count is the number of char's
   *which will be tested. StartAtIndex+Count is the last start position which
   *will be checked.
   * @return Return -1 if the String isn't part of this String else the start
   *position.
   **/
  Int32 IndexOf(const String& Value,
   ...
   * @return Return -1 if there is no positive comparement else the position of
   *the last sub String.
   **/
  Int32 LastIndexOf(const String& Value) const;

  /** @brief Search the last positive sub String in a String and start at
   *StartAtIndex.
   *
   * This function have the same behavior like IndexOf with the difference
   * that the search start on the end and stop on the start of the String.
   * StartAtIndex move the stop mark because the function search from the end to
   *the start.
   * @return Return -1 if there is no positive comparement else the position of
   *the last sub String.
   **/
   ...
   * that the search start on the end and stop on the start of the String.
   * StartAtIndex move the stop mark because the function search from the end to
   *the start. Count move the start mark, which means StartAtIndex+Count is the
   *first comparement.
   * @return Return -1 if there is no positive comparement else the position of
   *the last sub String.
   **/
  Int32 LastIndexOf(const String& Value,
                    const UInt32 StartAtIndex,
                    const UInt32 Count) const;
  ...
  /// Like Format method but restrict the formatting to a specified variadic
  /// list.
  static String FormatStrict(const String& Str, va_list ArgumentList);
  /// Like the Format method but instead of a String object you pass a string
  /// literal.
  template <int N>
...
#pragma warning(pop)
  ...
  RF_Type::Size
  MoveByGlyphs(const RF_Type::UInt8*& Buffer, const RF_Type::Size Glyphs) const;
  RF_Type::Size
  GetLength(const RF_Type::UInt8* CString, const RF_Type::Size CStringByteSize);
...
String::String(char const (&CString)[N])
{
  ...
  {  // use the pointer of the string literal instead of create a copy
    ...
    m_DynBuffer.m_Buffer = const_cast<RF_Type::UInt8*>(
        reinterpret_cast<const RF_Type::UInt8*>(&CString[0]));
