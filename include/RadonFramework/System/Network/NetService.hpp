...
public:
  ...
  static Collections::Array<RF_Net::NetworkAdapter> GetLocalInterfaces();
  ...
  static RF_Net::Error Accept(const SocketHandler& Listener,
                              SocketHandler& NewClient,
                              RF_Net::EndPoint& ClientEndPoint);
  ...
  static RF_Net::Error
  Bind(const SocketHandler Handler, RF_Net::EndPoint& LocalEP);
  ...
  static RF_Net::Error
  Connect(const SocketHandler Handler, const RF_Net::EndPoint& RemoteEP);

  /** This method places a socket at a state where it is
   * listening for an incoming connection.
   **/
  static RF_Net::Error
  Listen(const SocketHandler Handler, const RF_Type::UInt32 MaxWaitingClients);
  ...
  static RF_Net::Error
  ReceiveFrom(const SocketHandler Handler,
              RF_Mem::AutoPointerArray<RF_Type::UInt8>& Data,
  ...
  static RF_Net::Error Send(const SocketHandler Handler,
                            ...
                            RF_Type::UInt32* SendDataSize = nullptr);
  ...
  template <typename T>
  static RF_Net::Error
  GetSocketOption(const SocketHandler Handler,
  ...
  template <typename T>
  static RF_Net::Error
  SetSocketOption(const SocketHandler Handler,
                  const RF_Net::SocketOptionLevel OptionLevel,
                  const RF_Net::SocketOptionName OptionName,
                  const T& OptionValue);
  ...
  static RF_Net::Error
  SetBlockingMode(const SocketHandler Handler, const RF_Type::Bool NewValue);

private:
  // This method retrieves a socket option in generic way.
  template <typename T>
  static RF_Net::Error
  GetSocketOptionWrapper(const SocketHandler Handler,
                         const RF_Net::SocketOptionLevel OptionLevel,
                         const RF_Net::SocketOptionName OptionName,
                         T& OptionValue);
  ...
  SetSocketOption(const SocketHandler Handler,
                  const RF_Net::SocketOptionLevel OptionLevel,
                  const RF_Net::SocketOptionName OptionName,
                  const void* const OptionValue,
                  const RF_Type::UInt32 OptionLength);

  static RF_Net::Error
  AddMembership(const NetService::SocketHandler Handler,
                const RF_Net::MulticastRequest& OptionValue);
  static RF_Net::Error
  DropMembership(const NetService::SocketHandler Handler,
                 const RF_Net::MulticastRequest& OptionValue);
  static RF_Net::Error
  SetMulticastInterface(const NetService::SocketHandler Handler,
                        const RF_Net::IPAddress& OptionValue);
};

template <typename T>
RF_Net::Error
NetService::GetSocketOption(const NetService::SocketHandler Handler,
                            const RF_Net::SocketOptionLevel OptionLevel,
                            const RF_Net::SocketOptionName OptionName,
                            T& OptionValue)
{
...
template <typename T>
RF_Net::Error
NetService::GetSocketOptionWrapper(const NetService::SocketHandler Handler,
                                   const RF_Net::SocketOptionLevel OptionLevel,
                                   const RF_Net::SocketOptionName OptionName,
                                   T& OptionValue)
{
  RF_Mem::AutoPointerArray<RF_Type::UInt8> arr(new RF_Type::UInt8[sizeof(T)],
                                               sizeof(T));
  RF_Net::Error result =
...
template <typename T>
RF_Net::Error
NetService::SetSocketOption(const NetService::SocketHandler Handler,
                            const RF_Net::SocketOptionLevel OptionLevel,
                            const RF_Net::SocketOptionName OptionName,
                            const T& OptionValue)
{
  return SetSocketOption(Handler, OptionLevel, OptionName, &OptionValue,
                         sizeof(T));
}
