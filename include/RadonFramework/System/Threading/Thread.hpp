...
using CreateCallback = void* (*)(RF_Thread::Thread& Instance,
                                 RF_Type::UInt64& PID);
...
using SetPriorityCallback = void (*)(void* Data,
                                     RF_Thread::ThreadPriority::Type Value);
using GetPriorityCallback = RF_Thread::ThreadPriority::Type (*)(void* Data);
using SetAffinityMaskCallback =
    RF_Type::Bool (*)(void* Data, const RF_Collect::BitArray<>& NewMask);
using GetAffinityMaskCallback = RF_Type::Bool (*)(void* Data,
 ...
 * The function creates a new sub-process, execute the Instance->Run() function
 * and writes the thread id to PID. It will return if an error occurred or after
 * the thread started. Call PostConfigurationComplete afterwards to start the
...
/**
 * On some architectures it's necessary to configure the thread after it was
 * created. To ensure that the configuration was applied before execute
 * Instance->Run() the internal thread function will wait till this function was
 * called.
 */
