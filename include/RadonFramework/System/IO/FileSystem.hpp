...
{
using FileHandle = RF_Mem::PointerID;
using MemoryMappingHandle = RF_Mem::PointerID;
using FileWatcherHandle = RF_Mem::PointerID;
...
{
public:
  FileWatcherEvent() = default;
  FileWatcherEvent(const FileWatcherEvent& Args) = default;
  FileWatcherEvent& operator=(const FileWatcherEvent& Args) = default;
  WatcherChangeTypes::Type ChangeType = WatcherChangeTypes::Invalid;
...
void GetNotDispatchedFunctions(
    RadonFramework::Collections::List<RF_Type::String>& Result);
...
using StatCallback =
    RF_Mem::AutoPointer<RF_IO::FileStatus> (*)(const RF_Type::String& Path);
...
using ChangeModeCallback =
    RF_Type::Bool (*)(const RF_Type::String& Path,
                      const RF_IO::AccessMode::Type NewMode);
// memory mapping

using MapFileIntoMemoryCallback =
    MemoryMappingHandle (*)(const FileHandle& Handle);
...
using OpenFileCallback =
    FileHandle (*)(const RF_Type::String& Filepath,
...
using SeekFileCallback =
    RF_Type::UInt64 (*)(const FileHandle& Handle,
...
using GenerateTempFilenameCallback =
    RF_Type::String (*)(const RF_Type::String& Path);

/**
 * @Path system path
 * @FileSize size in bytes
 */
using CreatePreAllocatedFileCallback =
    RF_Type::Bool (*)(const RF_Type::String& Path,
                      const RF_Type::Size FileSize);
...
using ChangeDirectoryCallback =
    RF_Type::Bool (*)(const RF_Type::String& Destination);

using CreateDirectoryCallback = RF_Type::Bool (*)(const RF_Type::String& Path);

using DirectoryContentCallback =
    RF_Mem::AutoPointerArray<RF_Type::String> (*)(const RF_Type::String& Path);

// file watcher

using CreateFileWatcherCallback =
    FileWatcherHandle (*)(const RF_Type::String& Path);

using DestroyFileWatcherCallback = void (*)(FileWatcherHandle& Handle);

using WaitForFileWatcherCallback =
    RF_Type::Bool (*)(const FileWatcherHandle& Handle, FileWatcherEvent& Event);
...
using GetFileWatcherEventCallback =
    RF_Type::Bool (*)(const FileWatcherHandle& Handle, FileWatcherEvent& Event);

using SystemPathToUriCallback =
    RF_Type::Bool (*)(const RF_Type::String& SystemPath,
